...
Το `Ctrl+L` discovery και το network-scoped target catalog χρησιμοποιούν το pinned shared `WinRMDiscovery` 1.4.0. Το main menu διαβάζει μόνο local successful-connection history και το short-lived snapshot του τελευταίου explicit scan· δεν σκανάρει ούτε ελέγχει μαζικά saved PCs πριν εμφανιστεί. Χωρίς fresh snapshot ένα saved target είναι `not checked`. Μετά από explicit completed scan, το ίδιο row δείχνει ανεξάρτητα `online`, management-closed ή `offline`, ενώ εξακολουθεί να επιλύεται με hostname/MAC/last-IP evidence μόνο όταν επιλεγεί. Μετά την explicit επιλογή στόχου, το pinned `WinRMWorkshop` προσθέτει και επαληθεύει μόνο το exact hostname/IP στο client `TrustedHosts`, χωρίς δεύτερο prompt. Το authenticated session opening ανήκει στο pinned `WinRMConnection`: TCP preflight, έως τρεις bounded προσπάθειες με άμεσο status, transient-only retry και σαφή κατηγορία αποτυχίας. Μετά την πρώτη επιτυχημένη σύνδεση, το credential αποθηκεύεται ως Windows DPAPI profile για τον ίδιο Windows user και installation.
